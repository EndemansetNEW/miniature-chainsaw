...
public function getUserID($extID, $userName = "Undefined") {
    ...
    $query = $db->prepare("SELECT userID FROM users WHERE extID LIKE BINARY :id OR userID = :id");
    $query->execute([':id' => $extID]);
    if ($query->rowCount() > 0) {
        return $query->fetchColumn();
    }
    $query = $db->prepare("INSERT INTO users (isRegistered, extID, userName, lastPlayed, IP) VALUES (:register, :id, :userName, :uploadDate, :IP)");
    $ip = $this->getIP();
    $query->execute([
        ':id' => $extID,
        ':register' => $register,
	...
	public function songReupload($url, $author, $name, $accountID) {
		...
		if(filter_var($song, FILTER_VALIDATE_URL) == TRUE && substr($song, 0, 4) == "http") {
			...
			$size = round((int) $info['size'] / 1024 / 1024, 2);
		...
		if(!$type) {
			...
			foreach($songs AS &$customSongs) {
				...
				$library['songs'][$customSongs['ID']] = $gdpsLibrary['songs'][$customSongs['ID']] = [
					...
					'authorID' => (int)($serverIDs[null]. 0 .$folderID[$authorName]),
					'size' => (int) $customSongs['size'] * 1024 * 1024,
	...
	public function getLibrarySongInfo($id, $type = 'music', $extraLibrary = false) {
		...
		if($type == 'music') {
			...
			return ['server' => $song['server'], 'ID' => $id, 'name' => $song['name'], 'authorID' => $song['authorID'], 'authorName' => $author['name'], 'size' => round((int) $song['size'] / 1024 / 1024, 2), 'download' => $link, 'seconds' => $song['seconds'], 'tags' => $song['tags'], 'ncs' => $song['ncs'], 'artists' => $song['artists'], 'externalLink' => $song['externalLink'], 'new' => $song['new'], 'priorityOrder' => $song['priorityOrder']];
		} else {
			$SFX = $library['IDs'][$id];
			$token = $this->randomString(11);
			$expires = time() + 3600;
